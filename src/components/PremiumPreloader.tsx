...
type PreloaderMode = "full" | "curtain";
...
const hasCompletedFullPreloader = () => {
  ...
  try {
    return window.sessionStorage.getItem(completionStorageKey) === "true";
  } catch {
    return false;
  }
};

const markFullPreloaderComplete = () => {
  try {
...
const PremiumPreloader = ({
  ...
  const [mode] = useState<PreloaderMode>(() =>
    typeof window !== "undefined" && hasCompletedFullPreloader() ? "curtain" : "full",
  );
  ...
  }, [effectiveDurationMs, isCurtainOnly, safeExitDelay]);
  ...
  return (
    <motion.div
      ...
      aria-label={isExiting ? undefined : isCurtainOnly ? "Preparing page" : "Loading page"}
    ...
    >
      <span style={visuallyHiddenStyle}>
        {isCurtainOnly ? "Preparing page" : "Loading page"}
      </span>
